...
bool IsOccluded(UPrimitiveComponent* Comp, const DepthBuffer& depthBuffer, const FMatrix& viewProjMatrix)
{
    FBoundingBox box = GetWorldBox(Comp);
    FVector center = (box.min + box.max) * 0.5f;
    FVector4 center4 = FVector4(center.x, center.y, center.z, 1.0f);
    FVector4 clipPos = viewProjMatrix.TransformFVector4(center4);
    if (clipPos.a == 0)
        return true;
    clipPos = clipPos / clipPos.a;
    float u = clipPos.x * 0.5f + 0.5f;
    float v = 1.0f - (clipPos.y * 0.5f + 0.5f);
    float sceneDepth = depthBuffer.GetDepth(u, v);
    float compDepth = clipPos.z * 0.5f + 0.5f;
    return (compDepth > sceneDepth + 0.001f); 
}

TArray<UPrimitiveComponent*> KDTreeSystem::KDTreeCullVisible(
    ...
    const DepthBuffer& depthBuffer)
{
    ...
    // KD-Tree의 루트를 통해 프러스텀 컬링 수행 (모든 후보를 추출)
    if (Root)
    {
        Root->QueryFrustum(viewFrustum, candidates);
    }

    // 후보들을 카메라에서 가까운 순으로 정렬 (예: 중심의 거리 제곱 기준)
    candidates.Sort([&](const UPrimitiveComponent& A, const UPrimitiveComponent& B) {
        float distA = (A.GetWorldLocation() - viewFrustum.CameraPosition).SizeSquared();
        float distB = (B.GetWorldLocation() - viewFrustum.CameraPosition).SizeSquared();
        return distA < distB;
        });

    TArray<UPrimitiveComponent*> visible;
    for (UPrimitiveComponent* comp : candidates)
    {
        if (!IsOccluded(comp, depthBuffer, viewProjMatrix))
        {
            visible.Add(comp);
        }
    }
